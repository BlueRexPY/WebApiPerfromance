#!/usr/bin/env bash
# Quick benchmark: 8 services, concurrency=30, Orders endpoint
set -euo pipefail
...
for svc in $O; do
  ...
  for i in $(seq 1 60); do
    if curl -s -o /dev/null -w '%{http_code}' "http://127.0.0.1:${hp}/orders" 2>/dev/null | grep -q 200; then ok=1; break; fi
    sleep 1
  done
  ...
  ro=$("$BENCH" --mode rest --url "http://127.0.0.1:${hp}/orders" --name "$name" --concurrency "$CONC" --requests "$TOTAL" 2>/dev/null)
  rr=$(extract_rps "$ro"); rr="${rr:-0}"
  echo "  $ro" >&2
  rs=$(capture_stats "$svc"); rm=$(echo "$rs"|cut -d'|' -f1); rc=$(echo "$rs"|cut -d'|' -f2); rp=$(echo "$rs"|cut -d'|' -f3)

  go=$("$BENCH" --mode grpc --url "http://127.0.0.1:${gp}" --endpoint orders --name "$name" --concurrency "$CONC" --requests "$TOTAL" 2>/dev/null)
  ...
  mkdir -p "$RES/$dn"
  cat > "$RES/$dn/UnifiedOrders.md" <<MDEOF
...
cat > "$RES/Summary.UnifiedOrders.md" <<MDEOF
...
echo "=== DONE ==="
echo "Summary: $RES/Summary.UnifiedOrders.md"
